...
import { Printer, Edit, Trash2, ArrowLeft, PenTool, FileText } from 'lucide-react';
...
const ResolutionDetailPage = () => {
  ...
  const fetchRes = async () => {
    try { const r = await api.get(`/resolutions/${id}`); setRes(r.data); }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/resolutions/${id}`); alert('삭제되었습니다.'); navigate('/resolutions'); }
    catch (err) { alert('삭제에 실패했습니다.'); }
  };

  const handleSign = async (signatureData, targetRole) => {
    try {
      await api.post(`/resolutions/${id}/sign`, { role: targetRole || signingRole, signature: signatureData });
      setSigningRole(null);
      fetchRes();
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write([
      '<html><head><title>' + res.resNumber + '</title>',
      '<style>',
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate('/resolutions')} className="flex items-center gap-2 text-gray-500 hover:text-gray-700"><ArrowLeft size={18} /> 목록으로</button>
        <div className="flex gap-2">
          <Link to={`/documents/${doc.id}`} className="btn-secondary flex items-center gap-2"><FileText size={16} /> 원 품의서</Link>
          <button onClick={handlePrint} className="btn-primary flex items-center gap-2"><Printer size={16} /> PDF/인쇄</button>
          <button onClick={() => navigate(`/resolutions/${id}/edit`)} className="btn-secondary flex items-center gap-2"><Edit size={16} /> 수정</button>
                ...
                <div className="h-24 flex items-center justify-center">
                  {approval?.signature ? (
                    <div className="border rounded-lg p-2 bg-gray-50 w-full h-full flex flex-col items-center justify-center">
                      <img src={approval.signature} alt={role} className="max-h-12 mx-auto" />
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                      <button onClick={() => setSigningRole(role)} className="w-full py-2 border-2 border-dashed border-green-300 rounded-lg text-green-500 hover:border-green-500 hover:bg-green-50 text-sm font-medium">직접 서명</button>
                      {user?.stampImage && <button onClick={() => handleSign(user.stampImage, role)} className="w-full py-2 border-2 border-dashed border-blue-300 rounded-lg text-blue-500 hover:border-blue-500 hover:bg-blue-50 text-sm font-medium">도장 사용</button>}
                    </div>
                  ) : (
      ...
      <div className="card p-0 overflow-hidden" ref={printRef}>
        <div style={{ padding: '14mm', fontFamily: F, color: '#000' }}>
          <div style={{ textAlign: 'center', fontSize: '26pt', fontWeight: 'bold', letterSpacing: '24px', paddingLeft: '24px', marginBottom: '5mm', fontFamily: F }}>결 의 서</div>
          <table style={{ borderCollapse: 'collapse', width: '100%', fontFamily: F, tableLayout: 'fixed' }}>
            ...
            <tbody>
              <tr style={{ height: '13mm' }}><td colSpan={6} style={{ border: B, padding: '6px 8px', fontSize: '10pt' }}>결의번호 :&nbsp;&nbsp;{res.resNumber}&nbsp;&nbsp;&nbsp;&nbsp;(품의번호 : {doc.docNumber})</td></tr>
              ...
              <tr style={{ height: '25mm' }}>
                ...
                <td style={{ border: B, textAlign: 'center', fontSize: '11pt' }}>{res.author}</td>
              ...
              <tr><td colSpan={6} style={{ border: B, padding: '20px 12px', fontSize: '12pt', lineHeight: '160%', verticalAlign: 'top' }}>
                ...
                <div style={{ textAlign: 'center', marginTop: '100px', marginBottom: '20px' }}>위와 같이 결의하오니 재가하여 주시기 바랍니다.</div>
